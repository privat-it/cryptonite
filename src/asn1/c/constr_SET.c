        ...
        asn_app_consume_bytes_f *cb, void *app_key)
{
    ...
    if (t2m_build_own) {
        ...
        if (!t2m_build) {
            ASN__ENCODE_FAILED;    /* There are such platforms */
        }
        t2m_count = 0;
    } else {
        t2m_build = NULL;
...
SET_free(asn_TYPE_descriptor_t *td, void *ptr, int contents_only)
{
    ...
    for (edx = 0; edx < td->elements_count; edx++) {
        ...
        if (elm->flags & ATF_POINTER) {
            ...
            if (memb_ptr) {
                ASN_FREE(elm->type, memb_ptr);
            }
        } else {
            memb_ptr = (void *)((char *)ptr + elm->memb_offset);
            ASN_STRUCT_FREE_CONTENTS_ONLY_VOID(elm->type, memb_ptr);
        }
    }
        ...
        if (elm->flags & ATF_POINTER) {
            ...
            if (!memb_ptr) {
                ...
                ASN__CTFAIL(app_key, td, sptr,
                        ...
                        td->name, elm->name, FILE_MARKER, __LINE__);
                return -1;
            }
        } else {
            memb_ptr = (const void *)((const char *)sptr + elm->memb_offset);

...
static KdfHelper pbkdf1_data[] = {
    {
        ...
        20,
        {
            0xC8, 0x8E, 0x9C, 0x67, 0x04, 0x1A, 0x74, 0xE0,
            0x35, 0x7B, 0xEF, 0xDF, 0xF9, 0x3F, 0x87, 0xDD,
            0xE0, 0x90, 0x42, 0x14
        }
    },
...
#ifdef FULL_ATEST
    {
        ...
        8,
        {
            0x84, 0x75, 0xC6, 0xA8, 0x53, 0x1A, 0x5D, 0x27
        }
    },
    {
        PBKDF1_GOST_HASH_ID,
        ...
        32,
        {
            0x37, 0x09, 0xD0, 0x9A, 0x35, 0x74, 0x04, 0x5A, 0x52, 0xEE, 0x9A,
            0xC6, 0x5D, 0x62, 0x77, 0x28, 0x2E, 0x06, 0x5A, 0xBC, 0x59, 0x7A,
            0xB0, 0x33, 0x0B, 0xD2, 0x75, 0x44, 0x62, 0x30, 0xB6, 0xA6
        }
    }
#endif
};
...
void test_kdf_pbkdf1(KdfHelper *td)
{
    ByteArray *dk = NULL;
    ByteArray *data = ba_alloc_by_len(td->dkLen);
    ByteArray *salt = ba_alloc_from_str(td->salt);
    ...
    ASSERT_RET_OK(kdf_pbkdf1(td->pass, salt, td->iterations, td->dkLen, td->id, &dk));
    CHECK_EQUALS_BA(data, dk);
...
void atest_kdf(void)
{
    ...
    ATEST_CORE(pbkdf1_data, test_kdf_pbkdf1, sizeof(KdfHelper));

    if (err_count == error_count) {
        msg_print_atest("KDF", "[pbkdf2, pbkdf1]", "OK");
    } else {
